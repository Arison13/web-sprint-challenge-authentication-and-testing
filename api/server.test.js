...
describe("[POST] /register", () => {
  ...
  beforeEach(async () => {
    res = await request(server)
      ...
      .send({ username: "parce", password: "fairies" });
  });
  ...
  it("responds with newly created user", async () => {
    expect(res.body).toMatchObject({ id: 1, username: "parce" });
  });
});
...
describe("[POST] /login", () => {
  ...
  beforeEach(async() => {
    await request(server)
      .post('/api/auth/register')
      .send({ username: "marego", password: "12345" })
  ...
  it("has a message", async() => {
    expect(res.body.message).toBe('welcome, marego')
  });
